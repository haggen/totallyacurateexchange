...
app.post("/", async (ctx) => {
	...
	const portfolio = await database.get<z.infer<api.portfolios.Portfolio>>(
		...sql.q`SELECT * FROM portfolios WHERE id = ${data.portfolioId} AND userId = ${session.userId} LIMIT 1;`,
	);
	...
	if (data.type === "ask") {
		const holding = await database.get<z.infer<api.holdings.Holding>>(
			...sql.q`SELECT * FROM holdings WHERE portfolioId = ${data.portfolioId} AND stockId = ${data.stockId} LIMIT 1;`,
		);
	...
	return ctx.json(order, Status.Created);
});
...
app.get("/", async (ctx) => {
	...
	return ctx.json(orders, Status.Ok);
});
